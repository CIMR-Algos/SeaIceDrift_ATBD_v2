...
char progname[] = "icedrift_solve_simplex";
...
int main(int argc, char *argv[]) {
   ...
   if ( (argc > 2)) {
      fmerrmsg(progname,"Takes one (optional) parameter on command line.\n\tThe 'paramFile'.");
   ...
   setProcessingMethodString("simplex");
   ...
   float  radiusNeighbours  = 100; // a value of 100 gives the 8 neighbours
   ...
   setFilteringLevelString("lev0");
   setProductDescriptionString();
   ret = prepareAndWrite_icedriftProduct(&xdim,xbest,bestval,processingflag,pattern,leng,dire,
         latB,lonB,latE,lonE,navg,xavg,yavg,len_avg,len_diff,xstd,ystd,
         x_stddev,y_stddev,xy_corr,uncertaintyflag);
   if (ret) {
