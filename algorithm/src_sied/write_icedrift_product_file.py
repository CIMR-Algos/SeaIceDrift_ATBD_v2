...
fdspec = importlib.util.spec_from_file_location('fmtdate', '/home/emilyjd/sidrift_lowres/ice-tracking/src/helper_fcns.py')
fdmod = importlib.util.module_from_spec(fdspec)
fdspec.loader.exec_module(fdmod)
rsspec = importlib.util.spec_from_file_location('round_sig', '/home/emilyjd/sidrift_lowres/ice-tracking/src/helper_fcns.py')
rsmod = importlib.util.module_from_spec(rsspec)
rsspec.loader.exec_module(rsmod)
...
def write_icedrift(area_def,
    ...
    start_date_fmt = fdmod.fmtdate(start_date)
    end_date_fmt = fdmod.fmtdate(end_date)
...
#                         area_def.pixel_size_x, area_def.pixel_size_y))
    ...
    if unitsstr == 'm':
        ...
        unitconv = 1.0
    out_Ax = rsmod.round_sig(area_def.resolution[0]) * unitconv
    out_Ay = rsmod.round_sig(area_def.resolution[1]) * unitconv
    ...
    with Dataset(fnameandpath, 'w', format='NETCDF3_CLASSIC') as dataset:
        ...
        # TODO: Check which is necessary information for a properly
        ...
        # TODO - what should go in inst/platform for icedrift from winds?
        elif method == 'wind':
        ...
        c_out_projstr = ''
        # TODO: Check that this is a complete projstr
        if re.search('ease', area_def.area_id):
