...
int remap_ll2xy(double lat, double lon, projPJ *pj, double Ax, double Bx, double Ay, double By, double *x, double *y, short round) {
   ...
   G   = pj_fwd(G, pj);
   //G = proj_trans(pj, 1, G); // 1 for a forward transformation
...
/* WARNING: no checks on pj == NULL */
int remap_xy2ll(double x,double y, projPJ *pj, double Ax, double Bx, double Ay, double By, double *lat, double *lon) {
   ...
   G    = pj_inv(G, pj);
   //G = proj_trans(pj, -1, G); // -1 for inverse transformation
...
int remap_xy2xy(double *x,double *y,short round, 
      projPJ *pj1, double Ax1, double Bx1, double Ay1, double By1,
      projPJ *pj2, double Ax2, double Bx2, double Ay2, double By2) {
...
void transform_IceDrift_fromXY(double lat0, double lon0, double xdrift, double ydrift,
      float Ax,float Bx, float Ay, float By, projPJ *proj,
